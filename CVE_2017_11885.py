...
#dwVarID (sent by the client) is used as a pointer to an array of functions. The application does not check the boundary
#of the array so is possible to jump to specific portions of memory achieving remote code execution.
...
#Windows Server 2003. Exploitation in modern Windows OS version would probably require an additional vulnerability in order to
#bypass ASLR
